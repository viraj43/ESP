...
public class EspAnalyzerService {
    ...
    private static class RateLimitTracker {
        ...
        public long getTimeSinceLastRateLimit() {
            return System.currentTimeMillis() - lastRateLimitTime;
        }
    }
    ...
    public String analyzeESPByDateRange(String fromDateStr, String toDateStr) {
        try {
            ...
            log.append("🚀 Starting Enhanced ESP Analysis with improved rate limiting...\n\n");
            ...
            // 1) Fetch PRIMARY emails with enhanced rate limiting
            log.append("📧 Phase 1: Fetching PRIMARY replies (mode=emode_focused)...\n");
            List<EmailRow> primaryRows = fetchEmailsByTimeWindowEnhanced(fromDate, toDate, "emode_focused",
                    REPLY_TYPE_PRIMARY, rateLimitTracker, log);

            // 2) Wait longer between phases if we hit rate limits
            if (rateLimitTracker.hasHitRateLimit()) {
                long cooldownTime = 10000; // 10 seconds cooldown
                log.append("\n⏳ COOLDOWN: Waiting ").append(cooldownTime / 1000)
                        .append(" seconds between Primary and Other phases due to rate limiting...\n");
                safeSleep(cooldownTime, log);
                rateLimitTracker.reset(); // Reset rate limit tracker
            }

            // 3) Fetch OTHER emails with enhanced rate limiting
            log.append("\n📧 Phase 2: Fetching OTHER replies (mode=emode_others)...\n");
            List<EmailRow> otherRows = fetchEmailsByTimeWindowEnhanced(fromDate, toDate, "emode_others",
                    REPLY_TYPE_OTHER, rateLimitTracker, log);
            ...
            // 5) ESP code lookup with rate limiting
            if (!primaryRows.isEmpty()) {
                if (rateLimitTracker.hasHitRateLimit()) {
                    long espCooldown = 15000; // 15 seconds before ESP lookup
                    log.append("⏳ COOLDOWN: Waiting ").append(espCooldown / 1000)
                            .append(" seconds before ESP lookup due to rate limiting...\n");
                    safeSleep(espCooldown, log);
                }
    ...
    // Enhanced version with better rate limiting
    private List<EmailRow> fetchEmailsByTimeWindowEnhanced(LocalDate fromDateIST, LocalDate toDateIST,
                                                           String mode, String replyType,
        ...
        log.append("🕐 Time-window for ").append(replyType).append(" (").append(mode).append("): ")
        ...
        try {
            while (true) {
                ...
                Response response = fetchEmailsBatchWithEnhancedRetry(pageTrail, mode, rateLimitTracker, log);

                if (response == null) {
                    consecutiveFailures++;
                    log.append("❌ Failed to fetch ").append(replyType).append(" batch ").append(batch)
                            .append(" (").append(consecutiveFailures).append("/").append(MAX_CONSECUTIVE_FAILURES).append(")\n");

                    if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
                        log.append("❌ Too many consecutive failures for ").append(replyType).append(". Stopping this phase.\n");
                        break;
                    }

                    // Longer delay after failure
                    long failureDelay = 5000 * consecutiveFailures; // 5s, 10s, 15s
                ...
                if (response.getStatusCode() != 200) {
                    consecutiveFailures++;
                    log.append("❌ ").append(replyType).append(" API failed with status: ").append(response.getStatusCode()).append("\n");
                    continue;
                }
                ...
                log.append("✅ ").append(replyType).append(" batch ").append(batch).append(": added=").append(addedFromBatch)
                ...
                if (oldestInBatch != null && oldestInBatch.isBefore(windowStart.minusDays(5))) {
                    log.append("⏹️ Reached emails well before window for ").append(replyType).append(". Stopping.\n");
                    break;
                }
            ...
            log.append("❌ Error in ").append(replyType).append(" fetch: ").append(e.getMessage()).append("\n");
        }

        log.append("📊 ").append(replyType).append(" fetch complete: ").append(allEmails.size()).append(" emails\n");
        return allEmails;
    }
            ...
            try {
                ...
                if (response.getStatusCode() == 429) {
                    ...
                    if (attempt < MAX_RETRIES) {
                        ...
                        long delay = RATE_LIMIT_DELAY_MS * attempt * 2; // 10s, 20s, 30s
    ...
    // Enhanced ESP code fetching with rate limit awareness
    private Map<String, Integer> fetchEspCodesWithEnhancedRateLimit(List<EmailRow> primaryRows,
        ...
        for (String lead : unique) {
            ...
            if (rateLimitTracker.hasHitRateLimit()) {
                delay *= 2; // Double delay if we've hit rate limits
            }
            safeSleep(delay, log);
        }
        return out;
